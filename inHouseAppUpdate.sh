...
echo "Press u to update inventory again for the deivce group.\n"
...
if [  "$breakLoop" = 'b' ] ; then
    ...
    unset breakLoop
    break 2
elif [  "$breakLoop" = 'u' ] ; then
    echo "\nSending additional inventory update"
    echo ""
    unset breakLoop
    UpdateInventory
else
...
####################################################################################################
# 
# Jamf|PRO connection information entry & ensuring we can connect to the Jamf|PRO API
#
####################################################################################################
...
appNameComparison
appVersionComparison
    echo "$appCheck  yo  $appName_NO_WHITESPACE  and $appVersionCheck yo $appVersion_NO_WHITESPACE"
